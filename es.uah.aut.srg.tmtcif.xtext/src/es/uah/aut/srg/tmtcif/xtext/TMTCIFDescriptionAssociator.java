...
public class TMTCIFDescriptionAssociator {
	...
	public void linkDocumentation(EObject model) {
		...
		while(itor.hasNext()) {
			...
			if(grammarElement == slRule || grammarElement == mlRule) {
				...
				// We add an exception if we are talking about a model element
				if(commentSemantic == semantic && ((semantic instanceof TMTCIFModelElement) == false) &&
						x.getOffset() > NodeModelUtils.findActualNodeFor(semantic).getOffset())
					continue;
				
				if(semantic instanceof TMTCIFModelObject) {
					TMTCIFModelObject object = (TMTCIFModelObject) semantic;
				...
				else if(semantic instanceof TMTCIFModelElement) {
					TMTCIFModelElement object = (TMTCIFModelElement) semantic;
					// found sequence is documentation for semantic
					associateDescription(object, commentSequence);
					// need a new sequence, or the one just given away may be cleared
					commentSequence = Lists.newArrayList();
	...
	private void associateDescription(EObject object, List<INode> commentSequence) {
		...
		if (object instanceof TMTCIFModelElement) {
			((TMTCIFModelElement)object).setDescription(buf.toString());
		}
		else if (object instanceof TMTCIFModelObject) {
			((TMTCIFModelObject)object).setDescription(buf.toString());
		}
